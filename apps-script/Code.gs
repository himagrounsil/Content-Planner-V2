...
function getTasksData() {
  ...
  // Baca semua kolom termasuk reference, result, dan notes
...
function createTaskData(taskData) {
  ...
  // Lock mechanism untuk mencegah concurrent access
  ...
  try {
    ...
    console.log('createTaskData called with:', taskData);
    console.log('Current lastRow:', lastRow);
    ...
    if (lastRow >= DATA_START_ROW) {
      // Cari baris terakhir yang memiliki nomor task
      for (let row = lastRow; row >= DATA_START_ROW; row--) {
        const noValue = sheet.getRange(row, COLUMNS.NO + 1).getValue();
        const n = Number(noValue);
        if (!isNaN(n) && n > 0) { 
          lastNumber = n; 
          lastNumberRow = row; 
          break; 
    ...
    console.log('Last number found:', lastNumber, 'at row:', lastNumberRow);
    console.log('Writing new task at row:', writeRow, 'with number:', newNo);
    ...
    // Pastikan baris yang akan ditulis tidak menimpa data yang ada
    if (writeRow <= lastRow) {
      console.log('Warning: Writing at row', writeRow, 'which might overwrite existing data');
    }
    ...
    const result = {
      ...
      dueDate: formatDate(dueDate),
      dateLeft: sheet.getRange(writeRow, COLUMNS.DATE_LEFT + 1).getValue(),
      inProgress: taskData.inProgress || 'Not Done',
      reference: taskData.reference || '',
      result: taskData.result || '',
      notes: taskData.notes || ''
    };
    ...
    console.error('Error in createTaskData:', error);
    return { error: 'Failed to create task due to concurrent access' };
  } finally {
    lock.releaseLock();
  }
}
...
function updateTaskData(id, taskData) {
  ...
  try {
    ...
    console.log('updateTaskData called with id:', id, 'taskData:', taskData);
    
    for (let row = DATA_START_ROW; row <= lastRow; row++) {
      const noValue = sheet.getRange(row, COLUMNS.NO + 1).getValue();
      if (noValue == id) {
        console.log('Found task at row:', row);
        
        // Update all fields
        if (taskData.task !== undefined) {
          sheet.getRange(row, COLUMNS.TASK + 1).setValue(taskData.task);
          console.log('Updated task:', taskData.task);
        }
        if (taskData.platform !== undefined) {
          sheet.getRange(row, COLUMNS.PLATFORM + 1).setValue(taskData.platform);
          console.log('Updated platform:', taskData.platform);
        }
        if (taskData.format !== undefined) {
          sheet.getRange(row, COLUMNS.FORMAT + 1).setValue(taskData.format);
          console.log('Updated format:', taskData.format);
        }
        if (taskData.assignedTo !== undefined) {
          sheet.getRange(row, COLUMNS.ASSIGNED_TO + 1).setValue(taskData.assignedTo);
          console.log('Updated assignedTo:', taskData.assignedTo);
        }
        if (taskData.dueDate !== undefined) {
          const dueDate = new Date((taskData.dueDate || '') + 'T00:00:00');
          sheet.getRange(row, COLUMNS.DUE_DATE + 1).setValue(dueDate).setNumberFormat('yyyy-mm-dd');
          sheet.getRange(row, COLUMNS.DATE_LEFT + 1).setFormula(`=F${row}-TODAY()`);
          console.log('Updated dueDate:', taskData.dueDate);
        }
        if (taskData.inProgress !== undefined) {
          sheet.getRange(row, COLUMNS.IN_PROGRESS + 1).setValue(taskData.inProgress);
          console.log('Updated inProgress:', taskData.inProgress);
        }
        if (taskData.reference !== undefined) {
          sheet.getRange(row, COLUMNS.REFERENCE + 1).setValue(taskData.reference);
          console.log('Updated reference:', taskData.reference);
        }
        if (taskData.result !== undefined) {
          sheet.getRange(row, COLUMNS.RESULT + 1).setValue(taskData.result);
          console.log('Updated result:', taskData.result);
        }
        if (taskData.notes !== undefined) {
          sheet.getRange(row, COLUMNS.NOTES + 1).setValue(taskData.notes);
          console.log('Updated notes:', taskData.notes);
        }
    ...
    return { error: 'Failed to update task due to concurrent access' };
  } finally {
    lock.releaseLock();
  }
}
...
function formatDate(date) {
  ...
  console.log('Apps Script formatDate input:', date, 'type:', typeof date);
  
  const d = new Date(date);
  console.log('Apps Script Date object:', d);
  
  // Handle different date formats
  if (typeof date === 'string' && date.match(/^\d{4}-\d{2}-\d{2}$/)) {
    // If it's already in YYYY-MM-DD format, return as is
    console.log('Apps Script returning as-is:', date);
    return date;
  }
  
  // Use local timezone methods to preserve the original date
  const year = d.getFullYear();
  const month = String(d.getMonth() + 1).padStart(2, '0');
  const day = String(d.getDate()).padStart(2, '0');
  const result = `${year}-${month}-${day}`;
  console.log('Apps Script formatted result:', result);
